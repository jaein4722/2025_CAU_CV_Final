...
class DWSConv(nn.Module):
    ...
    def __init__(self, nIn, nOut, kSize, stride=1, padding=0, dilation=(1, 1), bn_acti=True, bias=False):
        super().__init__()
        self.depthwise = nn.Conv2d(nIn, nIn, kernel_size=kSize, stride=stride, padding=padding, dilation=dilation, groups=nIn, bias=bias)
...
class Conv(nn.Module):
    ...
    def forward(self, input):
        ...
        if self.bn_acti: return self.acti(self.bn(output))
        return output

# [전략 2] 아키텍처 강화를 위한 새로운 스템 블록
class StemBlock(nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        # 표준 Conv를 사용하여 초기 특징 추출을 안정화
        self.conv1 = Conv(in_channels, out_channels, kSize=3, stride=2, padding=1, bn_acti=True)
        self.conv2 = Conv(out_channels, out_channels, kSize=3, stride=1, padding=1, bn_acti=True)
        self.conv3 = Conv(out_channels, out_channels, kSize=3, stride=1, padding=1, bn_acti=True)
    def forward(self, x):
...
class TCA(nn.Module):
    ...
    def forward(self, input):
        ...
        return self.acti(self.bn(br + br1 + br2))
...
# --- 최종 제출 모델 ---
class submission_HWNetFemto_v1(nn.Module):
    def __init__(self, in_channels=3, num_classes=1):
        ...
        # [전략 2] Init_Block을 강화된 StemBlock으로 교체
        self.Stem = StemBlock(in_channels, C)
...
if __name__ == "__main__":
    num_classes = 21
    net = submission_HWNetFemto_v1(in_channels=3, num_classes=num_classes)
    p = sum(p.numel() for p in net.parameters() if p.requires_grad)
    print(f"Model: HWNet-Femto_v1")
