...
class CFDStabilizedModule(nn.Module):
    """CFD 안정성 모듈 - v7_lite에서 성공한 모듈 유지"""
...
class EnhancedMultiScaleModule(nn.Module):
    """VOC 성능 향상을 위한 Enhanced Multi-scale 모듈"""
    def __init__(self, channels):
        super().__init__()
        # Multi-scale branches for VOC
        self.branch1 = nn.Conv2d(channels, channels//4, kernel_size=1, bias=False)
        self.branch2 = SeparableConv2d(channels, channels//4, kernel_size=3, padding=1, bias=False)
        self.branch3 = SeparableConv2d(channels, channels//4, kernel_size=3, padding=2, dilation=2, bias=False)
        self.branch4 = SeparableConv2d(channels, channels//4, kernel_size=3, padding=4, dilation=4, bias=False)
...
# --- MicroNetv8 인코더 (균형잡힌 최적화) ---

class MicroNetV8Encoder(nn.Module):
    """MicroNetv8 인코더 - 균형잡힌 성능 최적화"""
    def __init__(self, in_channels):
        super().__init__()

        # 균형잡힌 채널 설계: 3 → 10 → 16 → 22
        self.downsample_1 = MicroDownsampleModule(in_channels, 10)
        self.downsample_2 = MicroDownsampleModule(10, 16)
        
        # CFD 안정성 + VOC 성능 모듈
        self.downsample_modules = nn.Sequential(
            CFDStabilizedModule(16),
            EnhancedMultiScaleModule(16)
        )
        
        self.downsample_3 = MicroDownsampleModule(16, 22)

        # Feature modules 복원: 4개 (rates: 1,2,4,6)
        rates = [1, 2, 4, 6]
        self.feature_modules = nn.Sequential(*[
            MicroResidualMultiDilationConvModule(22, rate, 0.08) for rate in rates
        ])
...
# --- 최종 제출 모델: MicroNetv8 (균형잡힌 최적화) ---
class submission_MicroNetv8(nn.Module):
    """MicroNetv8 - 균형잡힌 성능 최적화"""
    def __init__(self, in_channels, num_classes, interpolate=True):
        ...
        # 인코더 (균형잡힌 최적화)
        self.encoder = MicroNetV8Encoder(in_channels)

        # Auxiliary path with CFD stability
        self.aux_downsample = MicroDownsampleModule(in_channels, 10)
        self.aux_refine = CFDStabilizedModule(10)

        # 업샘플 블록
        self.upsample_1 = MicroUpsampleModule(22, 16)
        
        # Enhanced upsample modules
...
if __name__ == "__main__":
    model = submission_MicroNetv8(3, 21)
