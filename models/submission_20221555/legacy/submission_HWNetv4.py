...
class Conv(nn.Module):
    def __init__(self, nIn, nOut, kSize, stride, padding, dilation=(1, 1), groups=1, bn_acti=False, bias=False):
        ...
        if self.bn_acti:
            self.bn = nn.BatchNorm2d(nOut, eps=1e-3, momentum=0.01)
...
class DSConv(nn.Module):
    def __init__(self, nIn, nOut, kSize, stride, padding, dilation=(1, 1), bn_acti=False, bias=False):
        super().__init__()
        self.bn_acti = bn_acti
...
class TCA(nn.Module):
    def __init__(self, c, d=1, kSize=3, dkSize=3):
        ...
        self.dconv3x3 = DSConv(c, c, (dkSize, dkSize), 1, padding=(1, 1), bn_acti=True)
        self.ddconv3x3 = DSConv(c, c, (dkSize, dkSize), 1, padding=(1 * d, 1 * d), dilation=(d, d), bn_acti=True)
        self.bn = nn.BatchNorm2d(c, eps=1e-3, momentum=0.01)
        self.acti = nn.SELU(inplace=True)
...
class DecoderBlock(nn.Module):
    def __init__(self, in_channels, skip_channels, out_channels):
        ...
        self.conv = nn.Sequential(
            DSConv(in_channels // 2 + skip_channels, out_channels, 3, 1, padding=1, bn_acti=True),
            DSConv(out_channels, out_channels, 3, 1, padding=1, bn_acti=True)
        )
...
class submission_HWNetv4(nn.Module):
...
if __name__ == "__main__":
    net = submission_HWNetv4(in_channels=3, num_classes=21)
