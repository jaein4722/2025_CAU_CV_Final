...
# [개선점 1] 일반 Conv를 대체할 DSConv 모듈 추가
...
class Conv(nn.Module): # 1x1 Conv나 depthwise conv를 위해 유지
...
# [개선점 1] DownSamplingBlock을 DSConv 버전으로 교체
class DownSamplingBlock_DS(nn.Module):
...
# [개선점 1] TCA를 DSConv 버전으로 교체
class TCA_DS(nn.Module):
...
# [개선점 1] PCT를 TCA_DS 버전으로 교체
class PCT_DS(nn.Module):
...
class DecoderBlock_DS(nn.Module):
...
class submission_HWNetv3_depthwise(nn.Module):
...
if __name__ == "__main__":
    net = submission_HWNetv3_depthwise(in_channels=3, num_classes=21)
